<template>
  <el-container class="AddUser">
    <el-header>
      ...
      <div class="ty-header-div1" @click="getTabShow3"><p class="ty-headr-p"><span>用户特征</span></p></div>
    </el-header>
      ...
      <div class="ty-el-main-list1">
        ...
        <div class="ty-content">
          ...
          <div class="ty-content-div-list">
            ...
            <el-select v-model="formInlineRegion">
      ...
      <el-table
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button
              size="mini"
              @click="handleEditTable(scope.$index, scope.row.url)">下载
            </el-button>
            <el-button
              size="mini"
              type="danger"
...
//        三级联动
    ...
    methods: {
      ...
      getUserDetail() {
        ...
        axios({
          ...
          if (res.data.code == 0) {
            ...
            self.formInlineRegion = data.type.toString();
            ...
            var birthAddresss = data.birthAddress.split("&");
            if (birthAddresss.length == 4) {
              self.selectedOptions.push(birthAddresss[0]);
              self.selectedOptions.push(birthAddresss[1]);
              self.selectedOptions.push(birthAddresss[2]);
              self.addDetail1 = birthAddresss[3];
            } else if (birthAddresss.length == 3) {
              self.selectedOptions.push(birthAddresss[0]);
              self.selectedOptions.push(birthAddresss[1]);
              self.selectedOptions.push(birthAddresss[2]);
            }

            var lifeAddress = data.lifeAddress.split("&");
            if (lifeAddress.length == 4) {
              self.selectedOptions2.push(lifeAddress[0]);
              self.selectedOptions2.push(lifeAddress[1]);
              self.selectedOptions2.push(lifeAddress[2]);
              self.addDetail2 = lifeAddress[3];
            } else if (lifeAddress.length == 3) {
              self.selectedOptions2.push(lifeAddress[0]);
              self.selectedOptions2.push(lifeAddress[1]);
              self.selectedOptions2.push(lifeAddress[2]);
            }
      ...
      //编辑档案
      handleEditTable(index, url) {
        console.log(index, url);
        window.location.href = url + "&token=" + this.headers.token
      },
